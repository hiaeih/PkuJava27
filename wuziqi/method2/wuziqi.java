...
class ChessFrame extends JFrame implements ActionListener {
	...
	public ChessFrame() {
		...
		mbar.add(makeMenu(gameMenu, new Object[] { "开局", "模式", "先手顺序", "悔棋",
				null, "退出" }, this));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (arg.equals("关于"))
			JOptionPane.showMessageDialog(this, "五子棋游戏测试版本——By 江西财经大学08计一张帅",
					"关于", 0);
		if (arg.equals("退出"))
			System.exit(0);
	}
...
class ChessModel {
	...
	void computerDo(int flag) {
		...
		for (i = 0; i < width; i++) {
			for (j = 0; j < width; j++) {
				if (this.arrMapShow[i][j] == -1) {// 算法判断是否下子
...
/*
 * 类MainPanel主要完成如下功能：1、构建一个面板，在该面板上画上棋盘；2、处理在该棋盘上的鼠标事件（如鼠标左键点击、鼠标右键点击、鼠标拖动等）
 */
